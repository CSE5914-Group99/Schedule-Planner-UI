...
export class LandingComponent {
  ...
  form: Partial<ScheduleItem> = {
    type: 'class', title: '', date: '', start: '', end: '', location: '', notes: ''
  };
  ...
  recalc(){ /* 占位：当前左侧卡片显示 items.length * 2 */ }

  add() {
    if (!this.form.title || !this.form.date || !this.form.start || !this.form.end) return;
    const newItem: ScheduleItem = {
      id: uuid(),
      title: this.form.title!, date: this.form.date!,
      start: this.form.start!, end: this.form.end!,
      location: this.form.location, notes: this.form.notes,
